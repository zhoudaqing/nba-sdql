            ...
            request(options, function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    ...
                    for (var j = 0; j < teamsArray.length; j++) {
                        ...
                        if (argv.debug) { console.log(" Picks entry JSON: " + JSON.stringify(picksEntry)); }
...
function buildRequestUrl(orignalUrl, date) {
    ...
    var returnUrl = " http://api.sportsdatabase.com/nba/query.json?sdql=team%2Cline%2Co%3Ateam%40";
    returnUrl+=query;
...
function emailTeamsToBet(teamsToBet) {
    ...
    if (teamsToBet.picks.length === 0) {
        mySubject = "There are NO sdql teams to bet for " + date;
        body = "No teams found for betting today.";
    } else {
        mySubject = date + ": NBA sdql teams found to bet!!!";
        body = "<h1>Teams to Bet for " + date + "</h1>";
...
function printTeamsToBet(teamsToBet) {
    if (argv.debug) { console.log("TeamsToBet JSON: " + JSON.stringify(teamsToBet)); }
    console.log("Teams to Bet");
    console.log("============");
    for (var i = 0; i < teamsToBet.picks.length; i++) {
        // this for loop is to cylce through opponents to be sure not to print any collisions
        var collision = false;
        ...
        if (!collision || argv.showCollisions) {
            ...
            //     console.log("Multiple Hits: " + teamsToBet.picks[i].hits);
            // }
